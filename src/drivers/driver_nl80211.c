				  ...
				  const u8 *key, size_t key_len)
{
	...
	if (!(drv->capa.flags & WPA_DRIVER_FLAGS_KEY_MGMT_OFFLOAD)) {
		wpa_printf(MSG_DEBUG, "driver flag WPA_DRIVER_FLAGS_KEY_MGMT_OFFLOAD is not set\n");
		return 0;
	}
				      ...
				      const u8 *key, size_t key_len)
{
	...
	if (alg == WPA_ALG_PMK &&
		...
		ret = key_mgmt_set_key(drv, key, key_len);
		if (ret) {
			wpa_printf(MSG_ERROR, "%s: Failed to set key",  __FUNCTION__);
		}
		return ret;
	}
#endif
        ...
        struct wpa_driver_associate_params *params)
{
        ...
        if (!params->passphrase) {
			wpa_printf(MSG_ERROR, "Passphrase not set for SAE association");
		return -1;
        }
		key = params->passphrase;
		key_len = os_strlen(params->passphrase);

		ret = nl80211_set_conn_sae_keys(drv, key, key_len);
        return ret;
}
#endif /* CONFIG_BRCM_SAE */
...
#ifdef CONFIG_DRIVER_NL80211_BRCM
	if (drv->capa.flags & WPA_DRIVER_FLAGS_KEY_MGMT_OFFLOAD) {
		wpa_printf(MSG_DEBUG, "calling key_mgmt_set_key() to set PMK\n");
		if ((params->key_mgmt_suite == WPA_KEY_MGMT_SAE ||
			params->key_mgmt_suite == WPA_KEY_MGMT_FT_SAE) &&
			params->passphrase) {
			ret = key_mgmt_set_key(drv, (u8 *)params->passphrase, sizeof(params->passphrase));
			if (ret) {
				wpa_printf(MSG_DEBUG, "%s: failed to set passphease, ret %d\n", __FUNCTION__, ret);
			    return ret;
			}
			...
			params->key_mgmt_suite == WPA_KEY_MGMT_FT_PSK) &&
			params->psk) {
			ret = key_mgmt_set_key(drv, params->psk, 32);
			if (ret) {
				wpa_printf(MSG_DEBUG, "%s: failed to set psk, ret %d\n", __FUNCTION__, ret);
				return ret;
			}
		} else {
			wpa_printf(MSG_DEBUG, "%s: this key_mgmt_suite is not supported for offload\n", __FUNCTION__);
		}
	}
#endif /* CONFIG_DRIVER_NL80211_BRCM */

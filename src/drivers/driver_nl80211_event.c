	                             ...
	                             u32 subcmd, u8 *data, size_t len)
{
	...
	case BRCM_VENDOR_EVENT_PRIV_STR:
		...
		wpa_msg(drv->ctx, MSG_INFO, "%s",
			data);
	break;
#ifdef CONFIG_BRCM_SAE
	case BRCM_VENDOR_EVENT_SAE_KEY: {
		u16 pmkid_len = 0;
		wpa_printf(MSG_DEBUG, "**** %s : %d event_sae_key received\n", __func__, __LINE__);
        nla_for_each_attr(iter, (const struct nlattr *)data, len, rem) {
			wpa_printf(MSG_DEBUG, "**** %s : %d event_sae_key processing\n", __func__, __LINE__);
			...
			if (nla_type(iter) == BRCM_SAE_KEY_ATTR_PMK) {
				wpa_printf(MSG_DEBUG, "**** %s : %d event_sae_key PMK processing\n", __func__, __LINE__);
			...
			if (nla_type(iter) == BRCM_SAE_KEY_ATTR_PMKID) {
				wpa_printf(MSG_DEBUG, "**** %s : %d event_sae_key PMKID processing\n", __func__, __LINE__);

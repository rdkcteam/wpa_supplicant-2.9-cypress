...
#ifdef _SC_ENCRYPT_WPA_SUPPLICANT_WPA_KEY_
/*
*wpa_transcoding function implement escape of special characters
*input:need change characters;output:output change characters.Example "1$\2'3"4" change to "'1$\2'"'"'3"4'",
*Return output length
*/
int wpa_transcoding(char *input, char *output)
{
        char *pin=NULL;
        char *pout=NULL;
...
/*
*wpa_value_check function deal wpa value double quotes
*input:need check characters;example:"12345" change to 12345
*return length
*/
int wpa_value_check(char *input, char *output)
{
	char *pin = NULL;
        char *pout = NULL;
	int len = 0;
	if((NULL==input || NULL == output) && (2>strlen(input)))
	{
		return -1;
	}
        len=strlen(input)-1;
...
static void write_psk(FILE *f, struct wpa_ssid *ssid)
{
	...
	if (value == NULL)
		return;
#ifdef _SC_ENCRYPT_WPA_SUPPLICANT_WPA_KEY_
	memset(wps_value,0,sizeof(wps_value));
	memset(wpa_value,0,sizeof(wpa_value));
 	if(-1 != wpa_value_check(value,wpa_value))
	{
		wpa_transcoding(wpa_value,wps_value);
	}
	else
	{
		wpa_transcoding(value,wps_value);
	}
	sprintf(tmp_buf, "/usr/local/bin/wpa_key_encrypt %s  &", wps_value);
	pp = popen(tmp_buf, "r");
	if(pp)
	{
		memset(tmp_buf, 0, sizeof(tmp_buf));
		while(NULL != fgets(tmp_buf, sizeof(tmp_buf), pp))
        	{
            		fprintf(f, "\tpsk=%s\n", tmp_buf);
        	}
        	pclose(pp);
